...
func TestAccOctopusDeployNpmFeed(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		CheckDestroy:             testNpmFeedCheckDestroy,
		PreCheck:                 func() { TestAccPreCheck(t) },
		ProtoV6ProviderFactories: ProtoV6ProviderFactories(),
		Steps: []resource.TestStep{
			{
				Check: resource.ComposeTestCheckFunc(
					testNpmFeedExists(prefix),
				...
				Config: testNpmFeedBasic(localName, downloadAttempts, downloadRetryBackoffSeconds, feedURI, name, username, password),
			},
		},
	})
}

func testNpmFeedBasic(localName string, downloadAttempts int, downloadRetryBackoffSeconds int, feedURI string, name string, username string, password string) string {
	return fmt.Sprintf(`resource "octopusdeploy_npm_feed" "%s" {
		download_attempts = "%v"
...
func testNpmFeedExists(prefix string) resource.TestCheckFunc {
...
func testNpmFeedCheckDestroy(s *terraform.State) error {
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "octopusdeploy_npm_feed" {
			continue
		}

		feed, err := octoClient.Feeds.GetByID(rs.Primary.ID)
		if err == nil && feed != nil {
			return fmt.Errorf("NPM feed (%s) still exists", rs.Primary.ID)
		}
	}

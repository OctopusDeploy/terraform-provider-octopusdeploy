...
func (r *parentEnvironmentResource) Metadata(_ context.Context, _ resource.MetadataRequest, resp *resource.MetadataResponse) {
...
func (r *parentEnvironmentResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	parentEnvironment := expandParentEnvironment(ctx, plan, resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	fullParentEnvironment, err := parentenvironments.GetByID(r.Config.Client, plan.SpaceID.ValueString(), createdParentEnvironment.ID)
	if err != nil {
		resp.Diagnostics.AddError("Error fetching parent environment after create", err.Error())
		return
	}

	state := flattenParentEnvironment(ctx, fullParentEnvironment, resp.Diagnostics)
...
func (r *parentEnvironmentResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	newState := flattenParentEnvironment(ctx, parentEnvironment, resp.Diagnostics)
...
func (r *parentEnvironmentResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	state := flattenParentEnvironment(ctx, updatedParentEnvironment, resp.Diagnostics)
	resp.Diagnostics.Append(resp.State.Set(ctx, state)...)
}
...
func expandParentEnvironment(ctx context.Context, model schemas.ParentEnvironmentModel, diags diag.Diagnostics) *parentenvironments.ParentEnvironment {
	parentEnvironment := &parentenvironments.ParentEnvironment{
		Name:        model.Name.ValueString(),
		SpaceID:     model.SpaceID.ValueString(),
		Description: model.Description.ValueString(),
		Slug:        model.Slug.ValueString(),
		SortOrder:   int(model.SortOrder.ValueInt64()),
	}
	...
	if !model.AutomaticDeprovisioningRule.IsNull() && !model.AutomaticDeprovisioningRule.IsUnknown() {
		var adr schemas.AutomaticDeprovisioningRuleModel
		diags.Append(model.AutomaticDeprovisioningRule.As(ctx, &adr, basetypes.ObjectAsOptions{})...)
		if diags.HasError() {
			return nil
		}

		parentEnvironment.AutomaticDeprovisioningRule = &parentenvironments.AutomaticDeprovisioningRule{
			ExpiryDays:  int(adr.Days.ValueInt64()),
			ExpiryHours: int(adr.Hours.ValueInt64()),
		}
	}
...
func flattenParentEnvironment(ctx context.Context, parentEnvironment *parentenvironments.ParentEnvironment, diags diag.Diagnostics) schemas.ParentEnvironmentModel {
	result := schemas.ParentEnvironmentModel{
		ResourceModel: schemas.ResourceModel{
			ID: types.StringValue(parentEnvironment.ID),
		},
		...
		SortOrder:        types.Int64Value(int64(parentEnvironment.SortOrder)),
	}

	// Handle automatic_deprovisioning_rule
	if parentEnvironment.AutomaticDeprovisioningRule != nil {
		adrModel := schemas.AutomaticDeprovisioningRuleModel{
		...
		obj, d := types.ObjectValueFrom(ctx, attributeTypes, adrModel)
		diags.Append(d...)
		if diags.HasError() {
			return result
		}
		result.AutomaticDeprovisioningRule = obj
	} else {
		result.AutomaticDeprovisioningRule = types.ObjectNull(map[string]attr.Type{
			"days":  types.Int64Type,
			"hours": types.Int64Type,
		})
	}

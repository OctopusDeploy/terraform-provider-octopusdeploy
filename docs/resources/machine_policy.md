...
- `health_check_interval` (Number) The time between automatic health checks, in nanoseconds. Defaults to 24 hours (86400000000000). Set this to 0 to never run automatic health checks.
...
```shell
terraform import [options] octopusdeploy_machine_policy.<name> <machine-policy-id>
```
